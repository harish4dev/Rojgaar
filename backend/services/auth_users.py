...
from services.jwt_tokens import create_worker_token
...
async def resolve_user_after_otp(phone: str, role: str) -> dict:
    ...
    if role == "worker":
        ...
        if not existing:
            ...
            user = worker.model_dump()
            return {
                "success": True,
                "user": user,
                "is_new": True,
                "access_token": create_worker_token(user["id"], digits),
            }
        return {
            "success": True,
            "user": existing,
            "is_new": False,
            "access_token": create_worker_token(existing["id"], digits),
        }
